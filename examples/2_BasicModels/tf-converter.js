    ...
    ***************************************************************************** */
    ...
    var json$2 = [
        ...
        },
        {
            ...
            'dlOpName': 'queueSize',
            'category': 'control',
            'params': [{ 'tfInputIndex': 0, 'dlParamName': 'fifoQueueId', 'type': 'number' }]
        },
        {
    ...
    var OperationMapper = (function () {
                    ...
                    if (inputIndex === undefined) {
                        switch (param.type) {
                            ...
                            case 'shape':
                                ...
                                break;
                            case 'shape[]':
                                value = _this.getTensorShapeArrayParam(node.attr, param.tfParamName, param.defaultValue);
                                if (value === undefined && !!param.tfParamNameDeprecated) {
                                    value = _this.getTensorShapeArrayParam(node.attr, param.tfParamNameDeprecated, param.defaultValue);
                                }
                                break;
                            case 'dtype':
                                ...
                                break;
                            case 'dtype[]':
                                value = _this.getDtypeArrayParam(node.attr, param.tfParamName, param.defaultValue);
                                if (value === undefined && !!param.tfParamNameDeprecated) {
                                    value = _this.getDtypeArrayParam(node.attr, param.tfParamNameDeprecated, param.defaultValue);
                                }
                                break;
                            case 'tensor':
                            case 'tensors':
        ...
        OperationMapper.prototype.getDtypeArrayParam = function (attrs, name, def) {
            var param = attrs[name];
            if (param && param.list.type && param.list.type.length) {
                var result = [];
                for (var i = 0; i < param.list.type.length; i++) {
        ...
        OperationMapper.prototype.getTensorShapeArrayParam = function (attrs, name, def) {
            var param = attrs[name];
            if (param && param.list.shape && param.list.shape.length) {
                return param.list.shape.map(function (v) {
                    return v.dim.map(function (dim) { return (typeof dim.size === 'number') ? dim.size :
                        dim.size['toInt'](); });
                });
            }
            return def;
    ...
    var FIFOQueue = (function () {
        function FIFOQueue(dtypes, shapes, capacity, name, container) {
        ...
        Object.defineProperty(FIFOQueue.prototype, "isClosed", {
            get: function () {
                return this.closed_;
            },
            enumerable: true,
        ...
        FIFOQueue.prototype.close = function () {
            this.closed_ = true;
        };
        FIFOQueue.prototype.size = function () {
            return this.queue.length;
        };
        FIFOQueue.prototype.enqueue = function (element, dtype) {
            var _this = this;
            if (this.closed_) {
            ...
            var dtypes = [];
            element.forEach(function (tensor, index) {
                if (_this.size() === 0 && _this.shapes.length === 0) {
                    _this.shapes.push(tensor.shape);
                }
                else {
                    _this.assertShapesMatch(_this.shapes[index], tensor.shape, "Queue " + _this.name + " enqueue shape mismatch:");
                    tfc.util.assert(_this.dtypes[index] === tensor.dtype, "Queue " + _this.name + " enqueue Tensor datatypes mismatch: " +
                        (_this.dtypes[index] + " vs " + tensor.dtype + "."));
                }
                dtypes.push(tensor.dtype);
            });
            this.queue.push(element);
        };
        FIFOQueue.prototype.enqueueMany = function (elements) {
            var _this = this;
            if (this.capacity >= 0 &&
                elements.length > (this.capacity - this.queue.length)) {
                throw new Error("Number of equeueing elements exceeds Queue  " + this.name + " capacity.");
            }
            elements.forEach(function (element) { return _this.enqueue(element); });
        };
        ...
        FIFOQueue.prototype.dequeueMany = function (num) {
            ...
            for (var i = 1; i < num; i++) {
                ...
                for (var j = 0; j < current.length; j++) {
                    var temp = current[j].reshape([1].concat(this.shapes[j]));
                    result[j] = result[j].concat(temp, 0);
                }
            }
            return result;
        };
        FIFOQueue.prototype.dequeueUpTo = function (num, dtypes) {
            if (dtypes === void 0) { dtypes = []; }
            if (num <= 0) {
                throw new Error("Invalid number of dequeueing Queue " + this.name + ".");
            }
            var current = this.dequeue();
            ...
            for (var i = 1; this.queue.length && i < num; i++) {
                current = this.dequeue();
                for (var j = 0; j < current.length; j++) {
                    var temp = current[j].reshape([1].concat(this.shapes[j]));
                    result[j] = result[j].concat(temp, 0);
        ...
        FIFOQueue.prototype.assertShapesMatch = function (shapeA, shapeB, errorMessagePrefix) {
            if (errorMessagePrefix === void 0) { errorMessagePrefix = ''; }
            tfc.util.assert(this.arraysEqual(shapeA, shapeB), errorMessagePrefix + (" Shapes " + shapeA + " and " + shapeB + " must match."));
        };
        FIFOQueue.prototype.arraysEqual = function (n1, n2) {
                ...
                switch (_b.label) {
                    case 0:
                        ...
                        switch (_a) {
                            ...
                            case 'fifoQueue': return [3, 17];
                            case 'queueEnqueue': return [3, 18];
                            case 'queueDequeueUpTo': return [3, 19];
                            case 'queueSize': return [3, 20];
                        }
                        return [3, 21];
                    case 1: return [2, [
                    ...
                    case 16:
                        ...
                        return [2, []];
                    case 17:
                        dtypes = getParamValue('dtypes', node, tensorMap, context);
                        shapes = getParamValue('shapes', node, tensorMap, context);
                        capacity = getParamValue('capacity', node, tensorMap, context);
                        container = getParamValue('container', node, tensorMap, context);
                        queueName = getParamValue('name', node, tensorMap, context);
                        fifoQueue = new FIFOQueue(dtypes, shapes, capacity, queueName, container);
                        context.addFIFOQueue(fifoQueue);
                        return [2, [tfc.scalar(fifoQueue.id), tfc.scalar(1.0)]];
                    case 18:
                        queueEnqueueId = getParamValue('fifoQueueId', node, tensorMap, context);
                        components = getParamValue('components', node, tensorMap, context);
                        tCompoments = getParamValue('Tcomponents', node, tensorMap, context);
                        queueEnqueueQueue = context.getFIFOQueue(queueEnqueueId);
                        queueEnqueueQueue.enqueue(components, tCompoments);
                        return [2, [tfc.scalar(1.0)]];
                    case 19:
                        queueDequeueUpToId = getParamValue('fifoQueueId', node, tensorMap, context);
                        num = getParamValue('num', node, tensorMap, context);
                        dequeueUpToDtypes = getParamValue('dtypes', node, tensorMap, context);
                        dequeueUpToQueue = context.getFIFOQueue(queueDequeueUpToId);
                        return [2, dequeueUpToQueue.dequeueUpTo(num, dequeueUpToDtypes)];
                    case 20:
                        queueSizeId = getParamValue('fifoQueueId', node, tensorMap, context);
                        queueSizeQueue = context.getFIFOQueue(queueSizeId);
                        return [2, [tfc.scalar(queueSizeQueue.size(), 'int32')]];
                    case 21: throw TypeError("Node type " + node.op + " is not implemented");
                }
            });
        });
    }
    ...
    var ExecutionContext = (function () {
        function ExecutionContext(weightMap, tensorArrayMap, fifoQueueMap) {
            this.weightMap = weightMap;
            this.tensorArrayMap = tensorArrayMap;
            this.fifoQueueMap = fifoQueueMap;
        ...
        ExecutionContext.prototype.addFIFOQueue = function (fifoQueue) {
            this.fifoQueueMap[fifoQueue.id] = fifoQueue;
        };
        ExecutionContext.prototype.getFIFOQueue = function (id) {
    ...
    var GraphExecutor = (function () {
        ...
        GraphExecutor.prototype.execute = function (inputs, strictInputCheck, outputs) {
            ...
            var result = tfc.tidy(function () {
                var context = new ExecutionContext(_this._weightMap, tensorArrayMap, fifoQueueMap);
        ...
        GraphExecutor.prototype.executeAsync = function (inputs, outputs) {
            return __awaiter(this, void 0, void 0, function () {
                var _this = this;
                var tensorArrayMap, fifoQueueMap, context, outputNames, tensors, results, outputIds, inputIdArray, inputIds;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            tensorArrayMap = {};
                            fifoQueueMap = {};
                            context = new ExecutionContext(this._weightMap, tensorArrayMap, fifoQueueMap);
